...
class DevicesTest(unittest.TestCase):
    ...
    def test_realtek_alc256_any_subsystem_and_legacy_port(self):
        for port in ('[Out] Speaker', 'analog-output-speaker'):
            entry = devices.match(sink('HDA:10ec0256,1028087c,00100002 HDA:8086280b,80860101,00100000', port), self.table)
    ...
    def test_other_codecs_and_ports_are_rejected(self):
        ...
        self.assertIsNone(devices.match(sink('HDA:10ec0256,1028087c,00100002', '[Out] Headphones'), self.table))
    ...
    def test_user_entries_take_priority(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            path.write_text(json.dumps({'devices': [{'name': 'Mine', 'codec': '10ec0256',
                                                     'subsystem': '1028087c', 'device_file': '/x.nsx'}]}))
            table = devices.load(user_path=path)
            self.assertEqual(devices.match(sink('HDA:10ec0256,1028087c,0'), table)['name'], 'Mine')
            self.assertNotEqual(devices.match(sink('HDA:10ec0256,11111111,0'), table)['name'], 'Mine')
